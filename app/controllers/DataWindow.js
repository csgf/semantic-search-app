...
function loadMore(start){
	...
	for (var i = start; i < limit; i++) {
			...
			//Ti.API.info(documents[i].name_rep);
			...
			var thumb = Ti.UI.createImageView({
				...
				width : "24dp",
				height : "24dp",
				image : "http://grid.ct.infn.it/api/flags/" + tableData[i].country_code + ".png"
			});		
...
/*function updateAnnotation(_annotations, region){
	activityIndicator.show();
	var visibleAnnotations = [];
	var mapBounds = {};	
	
	if(region && _annotations){
	...
	visibleAnnotations = require("db").visibleAnnotations("data", mapBounds);
	
	if(!isNaN(visibleAnnotations)){
		alert(visibleAnnotations+ " annotations. Please zoom-in the map to see theme");
		activityIndicator.hide();
		return;
	};
	$.mapview.removeAllAnnotations();
	$.mapview.setAnnotations(visibleAnnotations);
	activityIndicator.hide();
};
*/
...
function doLoadData(){
	...
	net.getData(function(documents) {
		Ti.API.info("finito di caricare OADR");
		tableData = documents;
		...
		for (var i = 0; i < documents.length; i++) {
			var annotation = Alloy.Globals.Map.createAnnotation({
				...
				country_code : documents[i].country_code,
		...
		Ti.API.info("Numero di annotazioni in Data: " + annotations.length);
		require("db").insertDataRepos(annotations);
			
		//if (OS_IOS) {
			//$.mapview.setAnnotations(annotations);			

			$.mapContainer.mapview.tab = $.tab;
			$.mapContainer.mapview.index = 1;
		...
		loadMore(0);
		//$.tv.visible = true;
		Alloy.Globals.dataAnnotationsLoaded = true;
...
/*if (OS_IOS) {
	...
	$.mapview.addEventListener('regionchanged', function (evt) {	
		...
		setTimeout(function(){
			Ti.API.info("Numero di annotazioni : " + annotations.length);
			if(annotations.length)
				updateAnnotation(annotations, region);
		},500);
	});
...
function searchRepos(e){	
	...
	lastTimeout = setTimeout(function(){
		...
		require("db").searchDataRepos(keyword, function(repos){
			tableData = repos;
